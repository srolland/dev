					<!-- BEGIN #cateory-right .category-block -->
					<div id="category-right" class="category-block alignright">
						...
						<?php /* show category two list */
						$tz_cat_two_posts = new WP_Query(); $tz_cat_two_posts->query('caller_get_posts=1&posts_per_page=' . $tz_cat_two_number . '&cat=' . $category_id . ''); ?>
						<?php while ($tz_cat_two_posts->have_posts()) : $tz_cat_two_posts->the_post(); ?>
						...
						<?php endwhile; /* end category two list */ ?>
					
					<!-- END #cateory-right .category-block -->
					</div>