 ...
 */
add_action( 'widgets_init', 'tz_habs_list_tweets_widgets' );

/*
 * Register widget.
 */
function tz_habs_list_tweets_widgets() {
	register_widget( 'TZ_Habs_List_Tweet_Widget' );
}

/*
 * Widget class.
 */
class tz_habs_list_tweet_widget extends WP_Widget {

	/* ---------------------------- */
	/* -------- Widget setup -------- */
	/* ---------------------------- */
	
	function TZ_Habs_List_Tweet_Widget() {
	
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'tz_habs_list_tweet_widget', 'description' => __('A widget that displays the latest Habs tweets based on a list.', 'framework') );

		/* Widget control settings. */
		//$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'tz_ad_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'tz_habs_list_tweet_widget', __('Habs Latest List Tweets','framework'), $widget_ops, $control_ops );
	}
		...
		/* Display Latest Tweets */
		 ?>
			...
			<div id="twitter_div" class="clearfix">
				<ul id="twitter_update_habs_list">
					<li>&nbsp;</li>
				</ul>
			...
			<script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/js/twitter-handler.js"></script>
						<script type="text/javascript" src="https://api.twitter.com/1/lists/statuses.json?slug=habs&owner_screen_name=sebrolland&per_page=7&callback=twitterCallback3"></script>
